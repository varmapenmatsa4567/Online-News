...
class App():
    ...
    def insertUser(self, name, email, pswd):
        user = User.query.filter_by(email=email).first()
        if user:
            self.js.alert("User already exists")
        else:
            user = User(name=name, email=email, password=generate_password_hash(pswd))
            db.session.add(user)
            db.session.commit()
            self.js.alert("Sign Up Successful")

    def addPost(self, title, content, category, city):
...
def view():
    posts = Post.query.order_by(Post.date.desc()).all()
    return App.render(render_template("userView.html", posts=posts))
...
def userPosts():
    ...
    print(email)
    print(posts)
    return App.render(render_template("posts.html",posts=posts))
